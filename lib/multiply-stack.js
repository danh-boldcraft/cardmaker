...
class MultiplyStack extends Stack {
  constructor(scope, id, props) {
    ...
    if (config.environment === 'production' || config.environment === 'test') {
      const requiredVars = [
        'CT_MEMBERSTACK_PUBLIC_KEY',
        'CT_MEMBERSTACK_SECRET_KEY'
      ];
    ...
    const multiplyFunction = new lambda.Function(this, 'MultiplyFunction', {
      ...
      environment: {
        ...
        // Secrets from environment variables (set before deployment)
        MEMBERSTACK_PUBLIC_KEY: process.env.CT_MEMBERSTACK_PUBLIC_KEY || '',
        MEMBERSTACK_SECRET_KEY: process.env.CT_MEMBERSTACK_SECRET_KEY || ''
      }
