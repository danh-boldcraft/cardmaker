...
class CardmakerStack extends Stack {
  constructor(scope, id, props) {
    ...
    // Shopify environment-specific variable names
    const shopifyStoreDomainVar = `CM_SHOPIFY_${envPrefix}_STORE_DOMAIN`;
    const shopifyAccessTokenVar = `CM_SHOPIFY_${envPrefix}_ACCESS_TOKEN`;
    const shopifyVariantIdVar = `CM_SHOPIFY_${envPrefix}_CARD_VARIANT_ID`;
    const shopifyWebhookSecretVar = `CM_SHOPIFY_${envPrefix}_WEBHOOK_SECRET`;
    ...
    const multiplyFunction = new lambda.Function(this, 'MultiplyFunction', {
      ...
      environment: {
        ...
        // Shopify configuration (environment-specific)
        SHOPIFY_STORE_DOMAIN: process.env[shopifyStoreDomainVar] || '',
        SHOPIFY_ACCESS_TOKEN: process.env[shopifyAccessTokenVar] || '',
        SHOPIFY_CARD_VARIANT_ID: process.env[shopifyVariantIdVar] || '',
        SHOPIFY_WEBHOOK_SECRET: process.env[shopifyWebhookSecretVar] || '',
        ...
        PRINTIFY_CARD_BLUEPRINT_ID: process.env.CM_PRINTIFY_CARD_BLUEPRINT_ID || '1094',
        PRINTIFY_PRINT_PROVIDER_ID: process.env.CM_PRINTIFY_PRINT_PROVIDER_ID || '228',
        PRINTIFY_CARD_VARIANT_ID: process.env.CM_PRINTIFY_CARD_VARIANT_ID || '81866'
      }
    ...
    // Create /checkout resource for Shopify Draft Order creation
    const checkoutResource = api.root.addResource('checkout');

    // Add POST method to /checkout with moderate throttling
    ...
    // Create /webhooks/shopify/orders/paid resource for Shopify webhook
    const webhooksResource = api.root.addResource('webhooks');
    const shopifyWebhookResource = webhooksResource.addResource('shopify');
    const ordersWebhookResource = shopifyWebhookResource.addResource('orders');
    const paidWebhookResource = ordersWebhookResource.addResource('paid');

    // Add POST method for Shopify orders/paid webhook
    paidWebhookResource.addMethod('POST', lambdaIntegration, {
      throttling: {
        rateLimit: 5,   // Allow reasonable webhook throughput
        burstLimit: 10  // Shopify may send multiple webhooks
      }
    });
    ...
    new CfnOutput(this, 'CheckoutEndpoint', {
      value: `${api.url}checkout`,
      description: `Checkout endpoint URL (${config.environment})`
    });

    new CfnOutput(this, 'ShopifyWebhookEndpoint', {
      value: `${api.url}webhooks/shopify/orders/paid`,
      description: `Shopify orders/paid webhook URL (${config.environment})`
    });
